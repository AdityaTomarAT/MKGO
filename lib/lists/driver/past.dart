...
class _PastListState extends State<PastList> {
  ...
  void initState() {
    super.initState();
    print('present list ki length: ${trips.length}');
    tripListPast();
    setState(() {
      subLists.clear();
    });
  ...
  Future<void> _fetchAdminData() async {
    ...
    try {
      // subLists.clear();
      final List<Map<String, dynamic>> adminData = await tripListPast();
      setState(() {
        trips = adminData;
      });
      // Splitting 'admin' list into sublists of 10 items each
      for (int i = 0; i < trips.length; i += 10) {
        final endIndex = (i + 10 < trips.length) ? i + 10 : trips.length;
        subLists.add(trips.sublist(i, endIndex));
      }
  ...
  Future<List<Map<String, dynamic>>> tripListPast() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<dynamic> adminToday2 = box.read('tripList');
