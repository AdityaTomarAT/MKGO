...
class PanierDriverList extends StatefulWidget {
  const PanierDriverList({super.key});

  @override
  State<PanierDriverList> createState() => _PanierDriverListState();
}

class _PanierDriverListState extends State<PanierDriverList> {
  @override
  void initState() {
    super.initState();
    print('present list ki length: ${tripsPresent.length}');
    tripListPresent();
    setState(() {
      subLists.clear();
    });
  ...
  Future<void> _fetchAdminData() async {
    ...
    try {
      // subLists.clear();
      final List<Map<String, dynamic>> adminData = await tripListPresent();
      setState(() {
        tripsPresent = adminData;
      });
      // Splitting 'admin' list into sublists of 10 items each
      for (int i = 0; i < tripsPresent.length; i += 10) {
        final endIndex =
            (i + 10 < tripsPresent.length) ? i + 10 : tripsPresent.length;
        subLists.add(tripsPresent.sublist(i, endIndex));
      }
  ...
  Future<List<Map<String, dynamic>>> tripListPresent() async {
    setState(() {
      isRefreshed = true;
      print('Inside panier function for :');
    });
    ...
    final gestionMainUrl = gestionBaseUrl + "mob/panier";
    ...
    request.body = json.encode({"region": regionId});
...
// Inside the onPressed callback of the forward arrow IconButton
  void _updateNextSetOfItems() {
    int startIndex = currentPage * itemsPerPage;
    int endIndex = startIndex + itemsPerPage;
    if (endIndex <= tripsPresent.length) {
      nextSetOfItems = tripsPresent.sublist(startIndex, endIndex);
    } else {
      nextSetOfItems = tripsPresent.sublist(startIndex);
    }
    setState(() {
      currentPage++;
    });
  ...
  void _loadPreviousSetOfItems() {
    if (currentPage > 1) {
      ...
      nextSetOfItems =
          tripsPresent.sublist(startIndex, startIndex + itemsPerPage);
      setState(() {
        isRefreshed = true;
      });
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          Future<void> Panier1() async {
                            ...
                            final gestionMainUrl = gestionBaseUrl +
                                "api/affectation/course-new/" +
                                item['id'].toString();

                            print(
                                'Item Id: for panier trip update: ${item["id"]}');
                          ...
                          return Padding(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                // borderRadius: BorderRadius.circular(5),
                                border: Border(
                                  left: BorderSide(color:  HexColor("#00000")),
                                  top: BorderSide(
                                      color: HexColor("#00000"), width: 7),
                                  right: BorderSide(color: HexColor("#00000")),
                                  bottom: BorderSide(color: HexColor("#00000")),
                                ),
                                boxShadow: [
                                  BoxShadow(
