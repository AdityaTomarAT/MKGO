...
class _TodayListState extends State<TodayList> {
  ...
  Future<List<Map<String, dynamic>>> returnRide2() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        DriverIdAnnular = 0;
        print('Driver Id: $DriverIdAnnular');
        selectedDriverNotifier.value = "DRIVER";
        final driver = box.read('driverName') ?? "Null hai value";
                          ...
                          child: GestureDetector(
                            onTap: () {
                              showModalBottomSheet(
                                  ...
                                  builder: (context) {
                                    return StatefulBuilder(
                                        builder: (BuildContext context, imdex) {
                                      final box = GetStorage();
  ...
  Future<List<Map<String, dynamic>>> TripListToday() async {
    ...
    if (response.statusCode == 200) {
      ...
      box.write('AdminToday5', adminTodayAPI);
  ...
  int calculateItemCount() {
    ...
    return endIndex <= AdminToday.length ? endIndex : AdminToday.length;
  }
  ...
  void _loadFirstSetOfItems() {
    ...
    endIndex = endIndex.clamp(
        0, AdminToday.length); // Ensure endIndex is within the range
    nextSetOfItems = AdminToday.sublist(startIndex, endIndex);
  }
  ...
  Future<void> _fetchAdminData() async {
    ...
    try {
      // subLists.clear();
      final List<Map<String, dynamic>> adminData = await TripListToday();
      setState(() {
        AdminToday = adminData;
      });
      // Splitting 'admin' list into sublists of 10 items each
      for (int i = 0; i < AdminToday.length; i += 10) {
        final endIndex =
            (i + 10 < AdminToday.length) ? i + 10 : AdminToday.length;
        subLists.add(AdminToday.sublist(i, endIndex));
      }
...
// Inside the onPressed callback of the forward arrow IconButton
  ...
  //   int endIndex = startIndex + itemsPerPage;
  //   if (endIndex <= AdminToday.length) {
  //     nextSetOfItems = AdminToday.sublist(startIndex, endIndex);
  //   } else {
  //     nextSetOfItems = AdminToday.sublist(startIndex);
  //   }
  //   setState(() {
  //     currentPage++;
  //   });
  ...
  void _loadPreviousSetOfItems() {
    if (currentPage > 1) {
      ...
      int startIndex = (currentPage - 1) * itemsPerPage;
      nextSetOfItems =
          AdminToday.sublist(startIndex, startIndex + itemsPerPage);
      setState(() {
        isRefreshed = true;
      });
