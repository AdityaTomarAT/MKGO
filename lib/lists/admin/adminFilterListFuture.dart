// ignore_for_file: prefer_const_constructors, prefer_const_literals_to_create_immutables, use_build_context_synchronously, unused_field, unused_local_variable, deprecated_member_use, unused_element, no_leading_underscores_for_local_identifiers, avoid_print
...
List<bool> handOpenToday =
    List.generate(AdminListFilter.length, (index) => false, growable: true);

GetStorage box = GetStorage();

class AdminFilterFuture extends StatefulWidget {
  final String? data;
  AdminFilterFuture({super.key, this.data});

  @override
  State<AdminFilterFuture> createState() => _AdminFilterFutureState();
}

class _AdminFilterFutureState extends State<AdminFilterFuture> {
  // List<bool> handOpenToday = List.generate(AdminToday.length, (index) => false, growable: true);
  ...
  int DriverIdAnnular = box.read('driverIdAdmin') ?? 0;
  ...
  Future<List<Map<String, dynamic>>> TripListToday() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Map<String, dynamic>> AdminTodayAPI = [];
  ...
  int calculateItemCount1() {
    int itemCount = 0;

    itemCount += (currentPage * itemsPerPage);

    if (itemCount > AdminListFilter.length) {
      print('PresentList length: ${AdminListFilter.length}');
      itemCount = AdminListFilter.length;
  ...
  Widget build(BuildContext context) {
    // print('Bool in build function: $handOpenToday');
    // int boolLength = AdminToday.length;
    print('Is filte has data: ${widget.data}');
          ...
          child: isRefreshed
              ...
              : (widget.data == "")
                  ...
                  : ListView.builder(
                      ...
                      itemCount: appAdminFuture.isEmpty
                          ? 0
                          : appAdminFuture[currentIndex].length,
                      itemBuilder: (BuildContext context, int index) {
                        // Display each item in the sublist as a ListTile
                        final item = appAdminFuture[currentIndex][index];
            ...
            ? Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      //  - ${subLists.length}
                      '${currentIndex + 1} - ${appAdminFuture.length}',
                    ...
                    IconButton(
                      ...
                      onPressed: currentIndex < appAdminFuture.length - 1
                          ? () {
                              setState(() {
                                isRefreshed = true;
                              });
